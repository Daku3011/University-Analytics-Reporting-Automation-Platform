...
class BatchUploadFile(models.Model):
    CATEGORY_CHOICES = [
        ('social_media', 'Social Media'),
        ('digital_print', 'Digital Print'),
        ('physical_newspaper', 'Physical Newspaper'),
    ]
...
class AnnualCategoryReport(models.Model):
    CATEGORY_CHOICES = [
        ('all', 'Combined Report'),
        ('social_media', 'Social Media Only'),
        ('digital_print', 'Digital Print Only'),
        ('physical_newspaper', 'Physical Newspaper Only'),
    ]
    ...
    category = models.CharField(
        ...
        default='all',
        help_text="Selected category scope for this report"
    )
    ...
    output_pdf = models.FileField(
        upload_to='reports/annual_summaries/',
        blank=True, null=True,
        help_text="Generated PDF report file"
    )
    uploaded_by = models.ForeignKey(
        'auth.User', on_delete=models.SET_NULL, null=True, blank=True,
        related_name='annual_category_reports'
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"{self.title} ({self.get_category_display()} - {self.start_date} to {self.end_date})"
